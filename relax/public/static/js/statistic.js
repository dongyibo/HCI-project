...
function initLevelChart() {
    var success = function (data) {
        var levelChart = echarts.init(document.getElementById('level_chart'));

        var datax= [];
        var datay= [];
        for (var i in data){
            datax.push(data[i].level+"级");
            var tmp = {};
            tmp.value=data[i].number;
            tmp.name=data[i].level+"级";
            datay.push(tmp);
        }
        // alert(datay);
    ...
    $.ajax({
        type: 'GET',
        url: '/relax/public/ajax/userLevels',
        dataType: 'json',
        success: success,
